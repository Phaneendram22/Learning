...
sriov_vf_tenants=( "SANE"  "sane"  "URLB" "TROUVE" "rgst" "iGEMS")
...
function Setup_Additional_FS_GUESTOS ()
{
        ...
        # Check if any virtual disk is using disks in slot 3
...
function Setup_VFs ()
{
        ...
        echo "#_________________________________________________________________________"
        #
        wget --no-proxy -O /usr/local/bin/vf_setup.sh http://$ARCADIAN_REPO_IP/Arcadian/$TENANT/add-ons/vf_setup_UB22_760.sh
...
function Disable_PXE_NIC3 ()
{
	...
	echo "Setting boot proto on eno8303 to NONE"
...
function VF_Fix ()
{
        ...
        echo "After=network.target vf_setup.sh">>/etc/systemd/system/libvirtd.service.d/00-setup-vfs.conf
        echo "##########################################################">>/etc/systemd/system/libvirtd.service.d/00-setup-vfs.conf
