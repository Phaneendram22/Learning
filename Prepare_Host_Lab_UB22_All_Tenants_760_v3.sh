...
#======================================================================
#
#  Version: 3.0 : Prepare_Host_Lab_UB22_All_Tenants_760_v3.sh
#
#======================================================================
#
...
#  SIGNATURE:
#       Prepare_Host_Lab_UB22_All_Tenants_760_v3.sh
#
#======================================================================
#
...
#
#  Usage:
#       Run ./Prepare_Host_Lab_UB22_All_Tenants_760_v3.sh TENANT_NAME  as root.
#
...
#  DESCRIPTION:
#  :
#               Prepare_Host_Lab_UB22_All_Tenants_760_v3.sh scripts does the following features.
#
...
#This is only for Dell-760 servers
...
#Changed Disable_PXE_NIC3 to .7 as embedded NIC is NIC.NICConfig.7
...
function Installing_Cfengine ()
{
        echo "#_________________________________________________________________________"
        echo "#"
        echo "# Task: Started installing Cfengine for Host:$HOSTNAME at: `date`"
        echo "#"
        echo "#_________________________________________________________________________"
	cd /tmp; wget --no-proxy http://mirrors.it.att.com/pub/custom/SD/nasCommon/ubuntu/attcfengine2_2.2_amd64.deb
	dpkg -i attcfengine2_2.2_amd64.deb
        echo "#_________________________________________________________________________"
        echo "#"
        echo "# Task: Completed installing Cfengine for Host:$HOSTNAME at: `date`"
        echo "#"
        echo "#_________________________________________________________________________"
...
function Setup_Additional_FS_GUESTOS ()
{
        ...
        if echo "$racadm_output" | grep -w "Disk.Virtual.238:RAID.SL.3-1"; then
        echo "A virtual disk using disks in slot 3 already exists. No action taken."
        else
       # Create the virtual disk
       echo "Creating Raid 1 with disks 2 & 3 for VM disks"
       /bin/racadm storage createvd:RAID.SL.3-1 -rl r1 -wp wb -rp ra -name DATA2 -pdkey:Disk.Bay.2:Enclosure.Internal.0-1:RAID.SL.3-1,Disk.Bay.3:Enclosure.Internal.0-1:RAID.SL.3-1
       echo "Creating job for Raid 1 creation for VM disks"
       /bin/racadm jobqueue create RAID.SL.3-1 --realtime
       sleep 60
   fi
##################### Variables #############################
    DISK="/dev/sdb"  
...
function Setup_Additional_FS_GuestData() {
    ...
    if echo "$racadm_output" | grep -wq "Disk.Virtual.237:RAID.SL.3-1"; then
        echo "A virtual disk using disks in slot 3 already exists (Disk.Virtual.2). Proceeding to check partition and filesystem setup."
    else
        echo "Creating RAID-6 for VM DATA, please wait..."
        /bin/racadm storage createvd:RAID.SL.3-1 -rl r6 -wp wb -rp ra \
            -name DATA \
            -pdkey:Disk.Bay.4:Enclosure.Internal.0-1:RAID.SL.3-1,Disk.Bay.5:Enclosure.Internal.0-1:RAID.SL.3-1,Disk.Bay.6:Enclosure.Internal.0-1:RAID.SL.3-1,Disk.Bay.7:Enclosure.Internal.0-1:RAID.SL.3-1,Disk.Bay.8:Enclosure.Internal.0-1:RAID.SL.3-1,Disk.Bay.9:Enclosure.Internal.0-1:RAID.SL.3-1,Disk.Bay.10:Enclosure.Internal.0-1:RAID.SL.3-1,Disk.Bay.11:Enclosure.Internal.0-1:RAID.SL.3-1,Disk.Bay.12:Enclosure.Internal.0-1:RAID.SL.3-1,Disk.Bay.13:Enclosure.Internal.0-1:RAID.SL.3-1
        sleep 60
        ...
        for BAY in 14 15; do
            /bin/racadm storage hotspare:Disk.Bay.${BAY}:Enclosure.Internal.0-1:RAID.SL.3-1 -assign yes -type ghs
            sleep 120
        done
...
function Enable_SRIOV_NIC ()
{
        ...
        echo "#_________________________________________________________________________"
        #
        # Enabling SRIOV on eno1,2,enp55s0f0np0/1,enp139s0f0np0/1
	echo "# Task: Started setting up SRIOV on eno1 for Host:$HOSTNAME at: `date`"
        ...
        echo "# Task: Started setting up SRIOV on enp55s0f0 for Host:$HOSTNAME at: `date`"
        /bin/racadm set nic.DeviceLevelConfig.3.VirtualizationMode SRIOV
        echo "# Task: Started setting up SRIOV on enp55s0f1 for Host:$HOSTNAME at: `date`"
        /bin/racadm set nic.DeviceLevelConfig.4.VirtualizationMode SRIOV
        echo "# Task: Started setting up SRIOV on enp139s0f0 for Host:$HOSTNAME at: `date`"
        /bin/racadm set nic.DeviceLevelConfig.5.VirtualizationMode SRIOV
        echo "# Task: Started setting up SRIOV on enp139s0f1 for Host:$HOSTNAME at: `date`"
        ...
        echo "# Task: Started create jobqueue on enp55s0f0 for Host:$HOSTNAME at: `date`"
        /bin/racadm jobqueue create NIC.Slot.3-1-1
        echo "# Task: Started create jobqueue on enp55s0f1 for Host:$HOSTNAME at: `date`"
        /bin/racadm jobqueue create NIC.Slot.3-2-1
        echo "# Task: Started create jobqueue on enp139s0f0 for Host:$HOSTNAME at: `date`"
        /bin/racadm jobqueue create NIC.Slot.6-1-1
        echo "# Task: Started create jobqueue on enp139s0f1 for Host:$HOSTNAME at: `date`"
        /bin/racadm jobqueue create NIC.Slot.6-2-1
        sleep 90
...
function Setup_VFs ()
{
        ...
        echo "#_________________________________________________________________________"
        #
        wget --no-proxy -O /usr/local/bin/vf_setup.sh http://$ARCADIAN_REPO_INTLAB_IP/Arcadian/common/vf_setup_UB22_760.sh
        chmod +x /usr/local/bin/vf_setup.sh
        #
        wget --no-proxy -O /etc/systemd/system/vf_setup.service  http://$ARCADIAN_REPO_INTLAB_IP/Arcadian/common/vf_setup_UB22.service
...
function Disable_PXE_NIC3 ()
{
        echo "#_________________________________________________________________________"
        echo "#"
        echo "# Task: Started disabling PXE on eno8303 for Host:$HOSTNAME at: `date`"
        echo "#"
        echo "#_________________________________________________________________________"

	echo "Setting boot proto on eno3 to NONE"
	/bin/racadm set NIC.NICConfig.5.LegacyBootProto NONE
	...
	sleep 10
        ...
        echo "#_________________________________________________________________________"
        echo "#"
        echo "# Task: Completed disabling PXE on eno8303 for Host:$HOSTNAME at: `date`"
        echo "#"
        echo "#_________________________________________________________________________"
}
...
echo "Executing function Installing_Cfengine"
Installing_Cfengine

status=$( echo $? )
if [[ $status != 0 ]] ; then
	echo "Aborting script as Installing_Cfengine function failed"
	exit 0
fi
